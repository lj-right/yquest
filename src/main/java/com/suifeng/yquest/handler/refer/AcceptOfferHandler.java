...
/**
 * 接受Offer事件处理器：同步更新Offer状态为已接受
 */
@Slf4j
@Service
public class AcceptOfferHandler implements ReferEventHandler {
    ...
    public ReferApplyEventEnum getEvent() {
        return ReferApplyEventEnum.ACCEPT_OFFER;
    }
    ...
    public void apply(ReferApply referApply, ReferApplyStatusEnum fromStatus) {
        Offer offer = offerService.getOne(Wrappers.<Offer>lambdaQuery()
                ...
                .last("limit 1"));
        if (offer == null) {
            throw new IllegalStateException("Offer不存在，无法接受！");
        }
        offer.setStatus(1);
        offer.setUpdatedTime(new Date());
        offerService.updateById(offer);
        if (log.isInfoEnabled()) {
            log.info("接受Offer成功，申请ID{}，OfferID{}", referApply.getId(), offer.getId());
        }
    }
}
